...
interface ProductModalProps {
  isOpen: boolean;
  onClose: () => void;
  product?: MerchProps; // Passed when editing
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData?.id) {
      ...
      for (const key in formData) {
        if (
          formData[key as keyof MerchProps] !== product[key as keyof MerchProps]
        ) {
          updatedFields[key as keyof MerchProps] =
            formData[key as keyof MerchProps];
        }
      }
