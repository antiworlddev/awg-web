...
// import Similarmerchs from "@/app/ui/similar-merchs";
...
import SimilarMerch from "../_ui/similar-merch";
...
export default function Page(props: { params: Params }) {
  ...
  const { cart, setcart, all_merch, currency, exchangeRates } = context;
  ...
  // const merch = merchmerch;

  const merch = all_merch?.find((m) => m?.id === slug);
  ...
  const [selectedColor, setSelectedColor] = useState<ColorProps | undefined>({
    name: merch?.colors[0]?.name,
    hexCode: merch?.colors[0]?.hexCode,
    stock: merch?.colors[0]?.stock,
  });
  ...
  const viewMerch = (id: string) => {
    router.push(`/merch/${id}`);
  };

  // Handle decreasing merch quantity
  const handleLeftClick = () => {
    if (!isMerchInCart) return;

    const newCart = { ...cart };
    const merchInCart = newCart?.items[itemIndex];

    if (merchInCart.quantity > 1) {
      merchInCart.quantity -= 1;
    } else {
      // Remove merch from the cart when quantity is 0
  ...
  // Handle increasing merch quantity
  const handleRightClick = () => {
    ...
    if (
      selectedColor?.stock
        ? newCart?.items[itemIndex].quantity < selectedColor?.stock
        : undefined
    )
  ...
  const getPrice = () => {
    ...
    // If no part is selected, check if the merch has materialOptions
    if (selectedMaterial?.name) {
      return selectedMaterial.price;
    }

    // Fallback to merch base price
    return merch?.price ?? 0;
  };
  ...
  const addToBag = () => {
    if (merch) {
      ...
      let color = { ...merch?.colors[0] };

      if (merch?.colors?.length > 1) {
      ...
      const itemData: any = {
        item: {
          ...
          stock:
            merch?.colors?.find((c: any) => c?.name === color?.name)?.stock ??
            10,
  ...
  return (
    <div className={`flex flex-col w-full lg:px-24 px-8 text-black/80 `}>
      ...
      <div className="flex lg:flex-row  flex-col lg:items-start lg:justify-between items-center lg:space-x-4 w-full lg:mt-10">
        <div className="w-full">
          <Link href="/merch">
            <p className="underline text-xs uppercase"> ⬅︎ Back to store</p>
          </Link>
          ...
          >
            {merch?.images?.map((image: any, i: number) => (
              <SwiperSlide key={i}>
                <div className="lg:h-[640px] lg:w-[570px] w-full h-[440px] relative lg:mt-0 mt-8">
                  {image ? (
        ...
        <div className="flex flex-col lg:items-start items-center lg:w-2/5 w-full lg:mt-0 mt-10">
          <p className="lg:text-4xl text-2xl font-medium uppercase tracking-wide lg:text-left text-center">
            {merch?.name}
          </p>
          <p className={`mt-2 lg:text-lg font-light`}>
            {formatPrice(
              currency,
              getPrice() *
          ...
          <div className="flex w-full items-center justify-center">
            <div className="grid grid-cols-10 gap-1.5 self-start">
              {merch?.colors.map((color: any, i: number) => (
                <div
                  className={`${
          ...
          {/* {merch?.components?.length > 0 && (
            <PartSelector
              components={merch?.components}
              onSelectPart={(selected: any) => setSelectedPart(selected)}
              selectedPart={selectedPart?.name}
            />
          )}
          {merch?.materialOptions?.length > 0 && (
            <MaterialSelector
              materials={merch?.materialOptions}
      ...
      <SimilarMerch
        items={all_merch
          ?.filter((p: any) => p?.category === merch?.category)
          .filter((p: any) => p.id !== merch?.id)
          .slice(0, 4)}
        viewMerch={viewMerch}
      />
    </div>
